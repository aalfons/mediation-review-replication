...
methods <- c("ols_boot", "yj_boot", "ROBMED", "ryj_boot")
...
for (i in 1:nrow(settings)) {
  ...
  colors <- c("#F8766D", col_mix("#F8766D", "black", amount = 0.25),
              "#00BFC4", col_mix("#00BFC4", "black", amount = 0.25))
  symbols <- rep(21, 4)
  ...
  suffix <- paste(settings[i, "exposure"],
                   ...
                   sep = "_")
  if (settings[i, "b"] == 0) {
    file_plot <- "simulations/figures/figure_%s_transformations.pdf"
    # file_plot_bw <- "simulations/figures/figure_bw_%s_transformations.pdf"
  } else {
    file_plot <- "simulations/figures/figure_%s_transformations.pdf"
    # file_plot_bw <- "simulations/figures/figure_bw_%s_transformations.pdf"
  }
