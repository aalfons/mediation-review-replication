...
file_results <- "simulations/results/results_lavaan.RData"
load(file_results)
...
## methods to include
methods <- c("ml_boot", "ml_robust")

## nice labels to be used for plots
method_labels <- c(ml_boot = "ML-Boot",
                   ml_robust = "ML-Robust")
...
results <- results %>%
  ...
  mutate(estimate = if_else(str_detect(Method, "boot"),
                            estimate_boot, estimate_data))
...
for (i in 1:nrow(settings)) {
  ...
  if (settings[i, "a"] * settings[i, "b"] == 0) {
    ...
    current_results <- current_results %>%
      mutate(correct = reject_index & sign(estimate) == sign(a * b))
    # aggregate results
    df_reject <- current_results %>%
      group_by(exposure, a, b, Setting, Method) %>%
  ...
  df_ab <- current_results %>%
    group_by(exposure, a, b, Setting, Method) %>%
    summarize(Min = boxplot.stats(estimate)$stats[1],
              Lower = boxplot.stats(estimate)$stats[2],
              Middle = boxplot.stats(estimate)$stats[3],
              Upper = boxplot.stats(estimate)$stats[4],
              Max = boxplot.stats(estimate)$stats[5],
              .groups = "drop") %>%
    mutate(Parameter = "ab")
  ...
  suffix <- paste(settings[i, "exposure"],
                   ...
                   sep = "_")
  if (settings[i, "b"] == 0) {
    file_plot <- "simulations/figures/figure_%s_lavaan.pdf"
    # file_plot_bw <- "simulations/figures/figure_bw_%s_lavaan.pdf"
  } else {
    file_plot <- "simulations/figures/figure_%s_lavaan.pdf"
    # file_plot_bw <- "simulations/figures/figure_bw_%s_lavaan.pdf"
  }
